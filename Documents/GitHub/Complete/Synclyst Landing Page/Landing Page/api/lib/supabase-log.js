export async function logSynciqReturnEmail(email) {
  ...
  try {
    const res = await fetch(restUrl, {
      ...
      body: JSON.stringify({
        email,
        platform: 'synciq_return',
        source: 'mobile_hero_return',
      }),
    });
    ...
    console.warn('[supabase-log] synciq-return non-fatal error', res.status, text);
    return { ok: false };
  } catch (err) {
    console.warn('[supabase-log] synciq-return non-fatal exception', err.message);
    return { ok: false };
  }
}
